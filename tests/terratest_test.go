...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-smtp-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} SMTP Policy.",

	"Enabled": true,
	"MinSeverity": "critical",
	"SenderEmail": "ucs@example.com",
	"SharedScope": "",
	"SmtpPort": 25,
	"SmtpRecipients": [
	  "admin@example.com",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/smtp/Policies/%s", moid), expectedJSONTemplate, vars)
}
